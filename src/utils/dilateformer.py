...
class DilateAttention(nn.Module):
    ...
    def __init__(self, head_dim, qk_scale=None, attn_drop=0, kernel_size=3, dilation=1):
        ...
        self.unfold = nn.Unfold(kernel_size, dilation, dilation*(kernel_size-1)//2, 1) #将图片切割成不同的patch，但不进行卷积运算，也就是滑动窗口的实现
        # unfold的输出为（bs,Cxkernel_size[0]xkernel_size[1],L)， 其中L为patch的数量
        self.attn_drop = nn.Dropout(attn_drop)  #attn_drop=0,代表丢弃的概率为0

    def forward(self, q, k, v):
        #B, C//3, H, W
        #q,k,v分别代表transform领域中的键/值/查询
        B,d,H,W = q.shape
        q = q.reshape([B, d//self.head_dim, self.head_dim, 1, H*W]).permute(0, 1, 4, 3, 2)  # B,h,N,1,d ，permute：置换tensor的维度
        ...
        attn = (q @ k) * self.scale  # B,h,N,1,k*k, @表示常规的数学上定义的矩阵相乘；*表示两个矩阵对应位置处的两个元素相乘
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, H, W, d) # #transpose：维度置换
        return x
...
class Dilateformer(nn.Module):
    def __init__(self, img_size=64, patch_size=4, in_chans=3, num_classes=5, embed_dim=96,
...
@register_model
def dilateformer_tiny(img_size, num_class=5, pretrained=True, **kwargs):
    model = Dilateformer(img_size=img_size, num_classes=num_class, depths=[2, 2, 6, 2], embed_dim=72, num_heads=[ 3, 6, 12, 24 ], **kwargs)
    model.default_cfg = _cfg()  # 这是timm库里预训练好的模型
    return model
